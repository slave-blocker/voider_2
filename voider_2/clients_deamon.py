    ...
    if L[0] :    
        ...
        if not isSenator == '1' :
            ...
            while True :
                ...
                if  mymodule.isAlive(vps_ip_server, local_DoA_ssh_read_server) :
                    ...
                    if result[0] :
                        ...
                        subprocess.run(["iptables", "-t", "nat", "-A", "PREROUTING", "-i", mymodule.getint_out(), "-s", addr[0], "-j", "DNAT", "--to", '172.30.' + str(num) + '.2'])       
                        ...
                        subprocess.run(["ip", "netns", "exec", 'netns' + str(num), "iptables", "-t", "nat", "-A", "POSTROUTING", "-o", 'veth' + str(num), "-d", addr[0], "-j", "SNAT", "--to-source", mymodule.get_ip_address(mymodule.getint_out())])
                        ...
                        while connected :
                            ...
                            if mymodule.ping("172.31.0.1", num) :
                                ...
                                if count == 5 :
                                    ...
                                    subprocess.run(["iptables", "-t", "nat", "-D", "PREROUTING", "-i", mymodule.getint_out(), "-s", addr[0], "-j", "DNAT", "--to", '172.30.' + str(num) + '.2'])
                                    subprocess.run(["ip", "netns", "exec", 'netns' + str(num), "iptables", "-t", "nat", "-D", "POSTROUTING", "-o", 'veth' + str(num), "-d", addr[0], "-j", "SNAT", "--to-source", mymodule.get_ip_address(mymodule.getint_out())])
                                else :
            ...
            while True :
                ...
                subprocess.run(["iptables", "-t", "nat", "-A", "PREROUTING", "-i", mymodule.getint_out(), "-s", addr, "-j", "DNAT", "--to", '172.30.' + str(num) + '.2'])
                # this needs to be there because once the packets get into the default netns,
                # they will not get masquaraded out for some unknown reason :                
                subprocess.run(["ip", "netns", "exec", 'netns' + str(num), "iptables", "-t", "nat", "-A", "POSTROUTING", "-o", 'veth' + str(num), "-d", addr, "-j", "SNAT", "--to-source", mymodule.get_ip_address(mymodule.getint_out())])
                ...
                while connected :
                    ...
                    if mymodule.ping("172.31.0.1", num) :
                        ...
                        if count == 5 :
                            ...
                            L2 = [False, None]                        
                            # dinamic ip might have changed therefore need to delete iptable rules :
                            subprocess.run(["iptables", "-t", "nat", "-D", "PREROUTING", "-i", mymodule.getint_out(), "-s", addr, "-j", "DNAT", "--to", '172.30.' + str(num) + '.2'])
                            subprocess.run(["ip", "netns", "exec", 'netns' + str(num), "iptables", "-t", "nat", "-D", "POSTROUTING", "-o", 'veth' + str(num), "-d", addr, "-j", "SNAT", "--to-source", mymodule.get_ip_address(mymodule.getint_out())])
                        else :
                            print('ping ' + str(count) + ' failed')
                            time.sleep(5)
                            count = count + 1        
...
for line in Lines :
    if line[0] == '1' :
        threading.Thread(target=worker, args=(netns, phone, )).start()
    netns = netns + 1
